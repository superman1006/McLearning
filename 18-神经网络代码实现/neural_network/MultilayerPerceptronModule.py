...
class MultilayerPerceptron:  # 多层感知器
    def __init__(self, data, labels, layers, normalize_data=False):
        ...
        self.thetas = self.thetas_init(layers)

    def train(self, max_iterations=1000, alpha=0.1):
        # 将theta参数矩阵展开成一维向量
        unrolled_theta = self.thetas_unroll(self.thetas)

        (optimized_theta, cost_history) = \
            self.gradient_descent(self.data, self.labels, unrolled_theta, self.layers, max_iterations, alpha)

        # 将刚刚展开的一维向量转换成原来的参数矩阵
    ...
    def thetas_init(layers):
        ...
        for layer_index in range(num_layers - 1):
            # 会执行两次，得到两组随机参数矩阵：25*785 , 10*26
            in_count = layers[layer_index]
            out_count = layers[layer_index + 1]
            # 这里需要考虑到偏置项 b,记住一点偏置的个数跟输出的结果是一致的
    ...
    def thetas_unroll(thetas):
        ...
        for theta_layer_index in range(num_theta):
            unrolled_theta = np.hstack((unrolled_theta, thetas[theta_layer_index].flatten()))
        return unrolled_theta

    def gradient_descent(self, data, labels, unrolled_theta, layers, max_iterations, alpha):
        ...
        for i in range(max_iterations):
            # 计算损失函数
            cost = self.cost_function(data, labels, self.thetas_roll(optimized_theta, layers), layers)
            cost_history.append(cost)
            theta_gradient = self.gradient_step(data, labels, optimized_theta, layers)
            optimized_theta = optimized_theta - alpha * theta_gradient
            print(f'已经成功迭代{i}次')
        return optimized_theta, cost_history

    def gradient_step(self, data, labels, optimized_theta, layers):
        theta = self.thetas_roll(optimized_theta, layers)
        thetas_rolled_gradients = self.back_propagation(data, labels, theta, layers)
        thetas_unrolled_gradients = self.thetas_unroll(thetas_rolled_gradients)
    ...
    def cost_function(self, data, labels, thetas, layers):
        num_layers = len(layers)
        num_examples = data.shape[0]
        num_labels = layers[-1]

        # 前向传播走一次
        predictions = self.feedforward_propagation(data, thetas, layers)
